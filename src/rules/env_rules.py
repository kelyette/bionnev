...
class Rule1(EnvRule):
    def __init__(self):
        ...
        self.exp = "The cells each reproduce one new cell if they are a subrectangle of the grid (can be chosen to be any). Every new cell is then randomly put in the grid."    
        self.params_dict = {
        ...
        self.params_exp = {key: self.params_dict[key]["exp"] for key in self.params_dict.keys()}
        
    def env_func(_, env):
        reproduceable_cells = [cell for cell in env.cells if env.params['bottom']* cell.pos[env.params['horizontal']] >= env.params['bottom'] *int((env.size-1)*env.params['percent_grid'])]
...
class Rule2(EnvRule):
    ...
    def env_func(_, env):
        ### FONCTION REPRODUCEABLE PAS BONNE
        reproduceable_cells = [cell for cell in env.cells if env.params['bottom'] * cell.pos[env.params['horizontal']] >= env.params['bottom'] *int((env.size-1)*env.params['percent_grid'])]
        for cell in env.cells:
            cell.reproduceable = True if cell in reproduceable_cells else False
