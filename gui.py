...
def change_params(params, env_rule, cell_rule):
    ...
    needed_params = {list(params.keys())[i]: params[list(params.keys())[i]] for i in range(len(params)) if bool_params[i]}
    ...
    while True: 
        ...
        if params_event == sg.WIN_CLOSED or params_event == 'Exit':
            ...
            try: 
                new_value = int(params_values['value']) if int(params_values['value']) == float(params_values['value']) else float(params_values['value'])
                if isinstance(new_value, type(needed_params[selected_key])):
                    needed_params[selected_key] = params_values['value']
                else:
                    sg.popup_error(f"The value you inputted must be {type(needed_params[selected_key])}, not {type(new_value)}")
            except:
                sg.popup_error(f"The value you inputted cannot be transformed into a number.")
