...
def choose_rule(rule_dict, chosen_rule, title):
    rule_left_layout = [
        ...
        [sg.Listbox([rule for rule in rule_dict.keys()], default_values=chosen_rule,
                    key='selected_rule', select_mode=SELECT_MODE_SINGLE, size=(10, 10), enable_events=True)],
    ]
    rule_right_layout = [
        [sg.Text("Explanation", font=titlefont)],
        [sg.Multiline(rule_dict[chosen_rule]['exp'],
                      size=(40, 8), key='explanation')],
    ...
    while True:
        ...
        if rule_event == sg.WIN_CLOSED:
            ...
            chosen_rule = rule_values['selected_rule'][0]
            rule_window.Element('explanation').Update(
                value=rule_dict[chosen_rule]['exp'])
    rule_window.close()
...
def change_params(params, env_rule, cell_rule):
    ...
    params_left_layout = [
        ...
        [sg.Listbox([rule for rule in needed_params.keys()], default_values=selected_key,
                    key='selected_key', select_mode=SELECT_MODE_SINGLE, size=(10, 10), enable_events=True)],
    ]
    params_right_layout = [
        [sg.Text("Value", font=titlefont)],
        [sg.Text("Modify:"), sg.Input(
            default_text=needed_params[selected_key], key='value', enable_events=True)],
        [sg.Text("Explanation", font=titlefont)],
        [sg.Multiline(params_exp[selected_key],
                      size=(40, 8), key='explanation')],
    ...
    while True:
        ...
        if params_event == sg.WIN_CLOSED or params_event == 'Exit':
            ...
            params_window.Element('value').Update(
                value=needed_params[selected_key])
            params_window.Element('explanation').Update(
                value=params_exp[selected_key])
...
main_left_layout = [
    ...
    [sg.Text("Enviroment rule set:  ", font=font, size=(15, 1)), sg.Multiline(default_text=default_env_rule,
                                                                              font=font, key='chosen_env_rule', size=(15, 1), no_scrollbar=True), sg.Button("Choose", key='env_rule')],
    [sg.Text("Cell rule set:  ", font=font, size=(15, 1)), sg.Multiline(default_text=default_cell_rule,
                                                                        font=font, key='chosen_cell_rule', size=(15, 1), no_scrollbar=True), sg.Button("Choose", key='cell_rule')],
    [sg.Text("Parameters:  ", font=font, size=(15, 1)),
     sg.Button("Modify", key='params')],
...
window = sg.Window("Cells Evolution Simulator", main_layout,
                   finalize=True, font=font, resizable=True, location=(20, 20))
...
def main():
    while True:
        ...
        if event == sg.WIN_CLOSED:
            ...
            chosen_rule = choose_rule(
                env_rules, chosen_rule, "Environment Rule Selection")
            window.Element('chosen_env_rule').Update(value=chosen_rule)

        elif event == 'cell_rule':
            chosen_rule = values['chosen_cell_rule']
            chosen_rule = choose_rule(
                cell_rules, chosen_rule, "Cell Rule Selection")
            window.Element('chosen_cell_rule').Update(value=chosen_rule)

        elif event == 'params':
            change_params(
                params, values['chosen_env_rule'], values['chosen_cell_rule'])
